...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        # DONE: Lặp qua self.PII_PATTERNS.items()
        for pii_type, pattern in self.PII_PATTERNS.items():
            # DONE: Tìm tất cả matches
            matches = re.findall(pattern, value)

            for match in matches:
                # DONE: Thay thế match bằng "[PII_TYPE_REDACTED]" trong redacted_text
        ...
        if found_pii:
            ...
            # DONE: Trả về PassResult với value_override=redacted_text
            return FailResult(
                error_message=f"Phát hiện PII: {', '.join(p[0] for p in found_pii)}",
                fix_value=redacted_text,
            )

        # DONE: Không có PII → trả về PassResult với value gốc
...
class JSONFormatter(Validator):
    ...
    def _repair(text: str) -> str:
        ...
        # DONE: Thay single quotes → double quotes
        text = text.replace("'", '"')

        # DONE: Xóa trailing commas (dùng re.sub với r',\s*([}\]])' → r'\1')
    ...
    def validate(self, value: str, metadata: dict):
        ...
        """
        # DONE: Thử parse JSON trực tiếp
        try:
            parsed = json.loads(value)
            # DONE: Trả về PassResult với json.dumps(parsed, indent=2)
            return PassResult()
        except json.JSONDecodeError:
            pass

        # DONE: Thử sửa JSON rồi parse lại
        try:
...
def demo_pii_guard():
    ...
    # DONE: Tạo Guard với PIIDetector, truyền on_fail=OnFailAction.FIX vào CONSTRUCTOR
    ...
    for label, text in test_cases:
        # DONE: Gọi guard.validate(text) để lấy ValidationOutcome
...
def demo_json_guard():
    ...
    # DONE: Tạo Guard với JSONFormatter, truyền on_fail=OnFailAction.FIX vào CONSTRUCTOR
