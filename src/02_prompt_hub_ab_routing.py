...
# DONE: Đổi thành tên của bạn — phải là duy nhất trong Hub của bạn
PROMPT_V1_NAME = "lab22-rag-prompt-v1"
PROMPT_V2_NAME = "lab22-rag-prompt-v2"


# ── 2. Định nghĩa 2 Prompt Templates ──────────────────────────────────────
# DONE: Viết SYSTEM_V1 — phong cách ngắn gọn, trả lời 2-4 câu
...
# DONE: Viết SYSTEM_V2 — phong cách có cấu trúc, expert tone, 3-5 câu
...
def push_prompts_to_hub(client: Client):
    ...
    """
    # DONE: Push PROMPT_V1 — bọc trong try/except để xử lý lỗi
    try:
    ...
    # DONE: Push PROMPT_V2 — bọc trong try/except
    try:
...
def pull_prompts_from_hub(client: Client) -> dict:
    ...
    # DONE: Pull PROMPT_V1_NAME, fallback về PROMPT_V1 nếu lỗi
    try:
    ...
    # DONE: Pull PROMPT_V2_NAME, fallback về PROMPT_V2 nếu lỗi
    try:
...
def get_prompt_version(request_id: str) -> str:
    ...
    """
    # DONE: Tính MD5 hash của request_id và chuyển thành số nguyên
    hash_int = int(hashlib.md5(request_id.encode()).hexdigest(), 16)

    # DONE: Trả về PROMPT_V1_NAME nếu chẵn, PROMPT_V2_NAME nếu lẻ
    return PROMPT_V1_NAME if hash_int % 2 == 0 else PROMPT_V2_NAME
...
def ask_ab(retriever, llm, prompt, question: str, version: str) -> dict:
    ...
    """
    # DONE: Retrieve docs từ retriever
    docs = retriever.invoke(question)

    # DONE: Ghép page_content thành 1 string (dùng "\n\n".join)
    context = "\n\n".join(doc.page_content for doc in docs)

    # DONE: Chạy chain và lấy answer
    answer = (prompt | llm | StrOutputParser()).invoke({"context": context, "question": question})

    # DONE: Trả về dict kết quả
...
def main():
    ...
    # DONE: Tạo LangSmith Client với API key từ config
    # Gợi ý: client = Client(api_key=config.LANGSMITH_API_KEY)
    client = Client(api_key=config.LANGSMITH_API_KEY)

    # DONE: Push cả 2 prompts lên Hub
    push_prompts_to_hub(client)

    # DONE: Pull cả 2 prompts từ Hub (dùng dict trả về)
    prompts = pull_prompts_from_hub(client)

    # Tạo vectorstore, retriever và LLM
    vectorstore = setup_vectorstore()
    config.wait_for_gemini_embedding_quota()
    # DONE: Tạo retriever từ vectorstore (k=3)
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS):
        ...
        # DONE: Lấy version key từ request_id qua get_prompt_version()
        version_key = get_prompt_version(request_id)
        version_tag = "v1" if version_key == PROMPT_V1_NAME else "v2"
        prompt      = prompts[version_key]

        # DONE: Gọi ask_ab() với đúng arguments
